...
# Required Updated:
  # 1. variable names e.g. nCounts_RNA, nFeature_RNA, cclass, ctypes or celltypes
  # 2. Update specific names of the genes that need plot in feature plot and heatmap
  defaultGene = "SNAP25"
  heatMapGenes = "SATB2\nGAD2\nAQP4\nMOG\nMEGF11\nPTPRC\nFLT1"
  # # 3. Update the Title PanelId = <number> and Footer PanelId = <number>
  appNum = 1L
  # # 4. Update publication title and Pubmed ID
  pubRef = "https://pubmed.ncbi.nlm.nih.gov/37037607/"
  studyNam = "Cellular Diversity in Human Subgenual Anterior Cingulate and Dorsolateral Prefrontal Cortex by Single-Nucleus RNA-Sequencing"
  # # 5. Update and read "tour.txt" file
  tour <- read.delim("tour.txt", sep=";", stringsAsFactors = FALSE, row.names = NULL)
  # # 6. Update the dimension of the plots
...
numColor = as.numeric(length(unique(sce_small$celltype)))
n <- numColor
...
initial[["ComplexHeatmapPlot1"]] <- new("ComplexHeatmapPlot", Assay = "logcounts", CustomRows = TRUE, 
                                        ...
                                        DataBoxOpen = FALSE, VisualChoices = "Annotations", ColumnData = c("cclass", "celltype"), RowData = character(0), 
...
initial[["ColumnDataPlot1"]] <- new("ColumnDataPlot", XAxis = "Column data", YAxis = "nCount_RNA", 
                                    XAxisColumnData = "celltype", FacetRowByColData = "sample_pseudonym", 
                                    FacetColumnByColData = "sample_pseudonym", ColorByColumnData = "celltype", 
                                    ColorByFeatureNameAssay = "logcounts", ColorBySampleNameColor = "#FF0000", 
